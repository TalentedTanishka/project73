...
export default class readmoralStory extends React.Component{
  ...
  Search=async(text)=>{
    ...
    {
      const ref = await db.collection("moral-story").where('title','==',text).startAfter(this.state.dataSource).limit(10).get()
      ref.docs.map(doc =>{
        this.setState({
          allStories:[...this.state.allStories,doc.data()],
         dataSource:doc
    ...
    {
      const ref = await db.collection("moral-story").where('author','==',text).startAfter(this.state.dataSource).limit(10).get()
      ref.docs.map(doc =>{
        this.setState({
          allStories:[...this.state.allStories,doc.data()],
         dataSource:doc
    ...
    {
      const ref = await db.collection("moral-story").where('story','==',text).startAfter(this.state.dataSource).limit(10).get()
      ref.docs.map(doc =>{
        this.setState({
          allStories:[...this.state.allStories,doc.data()],
         dataSource:doc
 ...
 fontWeight:"bold",padding:20}}>
   ...
   <View>
   <SearchBar
  style={{borderWidth:3,borderRadius:10,marginTop:10,borderColor:"purple",fontSize:30,width:1000,fontFamily:"Comic Sans MS"}}
